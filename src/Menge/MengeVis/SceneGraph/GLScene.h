...
/*!
 @file    GLScene.h
 @brief   The definition of the scene -- the root of the directed, acyclic scene graph.

 It contains transforms, instances, lights, etc. It does *not* contain a camera. That is part of
 the viewer.
 */
...
class MENGEVIS_API GLScene {
 public:
  ...
  /*!
   ...
   This gives the scene the chance to free up memory and stop whatever processes it may have been
   using (e.g., flushing caches, etc.) Typically just called by the viewer at the conclusion of
   visualization. More particularly, it gives the scene's systems to clean up and conclude their
   processes.
   */
  void finish();

  /*!
   @brief   Allows the scene to recreate any unique OpenGL objects for all elements of the scene due
            to acquisition of a new OpenGL context.
   */
  void newGLContext();
  ...
  /*!
   ...
   A node is added to the scene. If no parent is specified, the node is a child of the scene
   directly. Otherwise, it is a child of the parent. NOTE: No test is performed to confirm that the
   parent already exists in the scene. It is the responsibility of the caller to confirm that this
   is the case.

   @param   node      The node to add to the scene.
   @param   parent    The optional parent node.
   @returns Indication if the addition was successful (true) or not (false). Currently, this
  ...
  /*!
   ...
   @returns True if the system is successfully added, false otherwise. In this implementation, it is
            *always* successful.
   */
  ...
  /*!
   ...
   @returns An instance of a ContextResult, reporting whether the event has been handled and whether
 ...
 private:
  ...
  /*!
   @brief   List of nodes which form the graph. Specifically, the nodes that are direct children of
            the scene.
   */
  GLNodeList _nodes;
